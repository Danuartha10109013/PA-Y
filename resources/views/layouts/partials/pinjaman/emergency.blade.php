...
<div class="table-responsive pt-3">
    <table class="table table-bordered">
        <thead bgcolor="EEEEEE">
            <h4>Tabel Pinjaman Emergency</h4>
            <tr>
                <th>
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" id="select-all-emergency">
                        <label class="custom-control-label" for="select-all-emergency"></label>
                    </div>
                </th>
        ...
        <tbody>
            @foreach ($pinjamanEmergency as $key => $data)
                @if ($data->status_bendahara == 'Diterima')
                    <tr>
                        <td>
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input checkbox-item"
                                    id="checkbox-emergency-{{ $data->id }}" data-id="{{ $data->id }}">
                                <label class="custom-control-label"
                                    for="checkbox-emergency-{{ $data->id }}"></label>
                            </div>
                        </td>
...
<script>
    document.getElementById('select-all-emergency').addEventListener('change', function() {
        var isChecked = this.checked;
        var checkboxes = document.querySelectorAll(
            'input[id^="checkbox-emergency-"]'); // Hanya checkbox Emergency
    ...
    document.querySelectorAll('input[id^="checkbox-emergency-"]').forEach(function(checkbox) {
    ...
    function toggleActionButtons() {
        var selectedCheckboxes = document.querySelectorAll('input[id^="checkbox-emergency-"]:checked');
