...
<div class="table-responsive pt-3">
    <table class="table table-bordered">
        <thead bgcolor="EEEEEE">
            <h4>Tabel Pinjaman Non angunan</h4>
            <tr>
                <th>
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" id="select-all-nonangunan">
                        <label class="custom-control-label" for="select-all-nonangunan"></label>
                    </div>
                </th>
        ...
        <tbody>
            @foreach ($pinjamanNonAngunan as $key => $data)
                @if ($data->status_bendahara == 'Diterima')
                    <tr>
                        <td>
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input checkbox-item"
                                    id="checkbox-nonangunan-{{ $data->id }}" data-id="{{ $data->id }}">
                                <label class="custom-control-label"
                                    for="checkbox-nonangunan-{{ $data->id }}"></label>
                            </div>
                        </td>
...
<script>
    document.getElementById('select-all-nonangunan').addEventListener('change', function() {
        var isChecked = this.checked;
        var checkboxes = document.querySelectorAll(
            'input[id^="checkbox-nonangunan-"]'); // Hanya checkbox non angunan
    ...
    document.querySelectorAll('input[id^="checkbox-nonangunan-"]').forEach(function(checkbox) {
    ...
    function toggleActionButtons() {
        var selectedCheckboxes = document.querySelectorAll('input[id^="checkbox-nonangunan-"]:checked');
