                    ...
                    <tbody>
                        @foreach ($simpanans as $simpanan)
                            <tr>
                                ...
                                <td>{{ $simpanan->invoice_number }}</td>
                                <td>{{ $simpanan->user->name }}</td>
                                <td>{{ $simpanan->virtual_account_number }}</td>
                                <td>Rp. {{ number_format($simpanan->amount, 2) }}</td>
                                ...
                                <td class="action-icons">
                                    @if ($simpanan->status == 'Menunggu')
                                        <form action="{{ route('simpanan.approve', $simpanan->id) }}" method="POST"
                                            style="display: inline;">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="btn btn-sm btn-success">Approve</button>
                                        </form>
                                        <form action="{{ route('simpanan.reject', $simpanan->id) }}" method="POST"
                                            style="display: inline;">
                                            @csrf
                                            @method('PUT')
