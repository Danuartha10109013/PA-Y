    ...
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.0/dist/css/bootstrap.min.css" rel="stylesheet">
        ...
        <style>
            ...
            .btn:disabled {
                ...
                opacity: 0.6; /* Membuat tombol terlihat tidak aktif */
            }
            .dropdown-menu {
            ...
            .input-group-text i {
                color: #6c757d; /* Warna ikon pencarian */
            }
            .input-group {
                width: 300px;
                margin-right: 10px;
                border-radius: 5px; /* Membuat tepi lebih halus */
            }
        </style>
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@4.6.0/dist/js/bootstrap.bundle.min.js"></script>
 ...
 <div class="content-background">
    <div class="pl-2 pb-8">
        <h3 class="font-weight-bold">Approval Registrasi Anggota</h3>
    </div>
            <div class="search-bar d-flex align-items-center">
    <div class="input-group" style="width: 300px;">
        <input type="text" class="form-control" placeholder="search" aria-label="Search" aria-describedby="search-icon" id="searchInput" oninput="searchData()" />
 ...
 <!-- Ikon pencarian -->
                ...
                <div class="dropdown mr-2">
                    <button class="btn btn-outline-secondary dropdown-toggle" type="button" id="filterDropdownMenu" data-bs-toggle="dropdown" aria-expanded="false">
                        Filter Status
                    </button>
                    <ul class="dropdown-menu" aria-labelledby="filterDropdownMenu">
                        ...
                        <li><a class="dropdown-item" href="javascript:void(0);" onclick="filterData('Diterima')">Diterima</a></li>
                        <li><a class="dropdown-item" href="javascript:void(0);" onclick="filterData('Pengajuan')">Belum Diterima</a></li>
                        <li><a class="dropdown-item" href="javascript:void(0);" onclick="filterData('Ditolak')">Ditolak</a></li>
                    </ul>
                </div>

                <div class="ml-auto d-flex">
                    <button id="acceptButton" type="submit" class="btn btn-success" onclick="updateStatus('Diterima')" disabled>Terima</button>
                    <button id="rejectButton" type="submit" class="btn btn-danger" onclick="updateStatus('Ditolak')">Tolak</button>
                </div>
            ...
            <div class="modal-body">
                <form id="editForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="editAlamatDomisili" name="alamat_domisili" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" class="form-control" id="editEmailKantor" name="email_kantor" required>
                    </div>
                    <div class="form-group">
                        <label for="editNoHandphone">No Handphone</label>
                        <input type="text" class="form-control" id="editNoHandphone" name="no_handphone" required>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        selectedCheckboxes.forEach(checkbox => {
                            ...
                            $.ajax({
                                url: "{{ route('approve.diterima-bendahara', ['id' => 'ID', 'status' => 'Diterima']) }}"
                    .replace('ID', id),
                    type: "POST",
                    data: {
                    _token: "{{ csrf_token() }}",
...
const id = checkbox.getAttribute('data-id');
$.ajax({
url: "{{ route('approve.ditolak-bendahara', ['id' => 'ID', 'status' => 'Ditolak']) }}"
.replace('ID', id),
type: "POST",
data: {
_token: "{{ csrf_token() }}",
...
$('#rejectReasonModal .btn-secondary').on('click', function () {
    console.log("Tombol 'Tutup' diklik");
});
$('#rejectReasonModal .btn-secondary').on('click', function () {
    $('#rejectReasonModal').modal('hide'); // Tutup modal
});
$('#rejectReasonModal .close').on('click', function () {
    console.log("Tanda X diklik");
});
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                selectedCheckboxes.forEach(checkbox => {
                    const id = checkbox.getAttribute('data-id');
                    $.ajax({
                        url: "{{ route('approve.update-status-bendahara', ['id' => 'ID', 'status' => 'STATUS']) }}"
                            .replace('ID', id)
    ...
    $(document).on('click', '.save-reject', function () {
        ...
        $.ajax({
            url: `/bendahara/anggota/reject/${id}`, // Endpoint Laravel
            type: "POST",
            data: {
            ...
            error: function (xhr) {
                ...
                Swal.fire("Error!", "Terjadi kesalahan saat menolak anggota.", "error");
            }
        });
    });
});
...
<script>
    ...
    $.ajax({
        url: `/bendahara/data/filter`, // URL endpoint filter
        type: 'GET',
        data: { status: status }, // Kirim status sebagai parameter query
        success: function(response) {
...
function searchData() {
    ...
    $.ajax({
        url: '/bendahara/data/search', // Endpoint Laravel
        type: 'GET',
        data: { query: query }, // Kirim query pencarian sebagai parameter
        success: function(response) {
...
function deleteData(element) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: `/bendahara/data/delete/${dataId}`, // Endpoint Laravel untuk menghapus data
                type: 'DELETE',
                data: {
    ...
    function viewDetail(element) {
        ...
        $.ajax({
            url: `/bendahara/anggota/detail/${dataId}`, // Endpoint Laravel untuk mengambil data
            type: 'GET',
            success: function (response) {
...
function openEditModal(element) {
    ...
    $.ajax({
        url: `/bendahara/anggota/detail/${dataId}`, // Endpoint untuk mendapatkan data anggota
        type: 'GET',
        success: function (response) {
...
function saveEdit() {
    ...
    $.ajax({
        url: `/bendahara/anggota/update/${dataId}`, // Endpoint untuk menyimpan data
        type: 'PUT',
        data: formData,
        success: function (response) {
